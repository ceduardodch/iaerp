...
| Codex (este hilo, SRI multiempresa) | 🚀 EN PROMOCIÓN | `--all` procesa DATA-CLIP, BTOB SAS y LEXCODE con RUC/clave, cuenta IAERP y perfil de Chrome separados. Evidencia previa: 468 registros en DATA-CLIP y 18 en BTOB. LEXCODE quedó provisionada con cuenta `tax:write`; dos corridas reales listaron las mismas cuatro facturas de agosto y la UI conservó cuatro compras, sin duplicarlas. Las tres encolan recuperación XML. El preflight RUC↔tenant y la validación local de todos los receptores bloquean cuentas o sesiones cruzadas antes de guardar evidencia. Diez pruebas Node y revisión de seguridad GO; tarea diaria activa a las 08:00; promoción autorizada. | 2026-08-31 |
| Codex (este hilo, razón social XML SRI) | 🚀 EN PROMOCIÓN | El SRI rechazó el valor `LEXCODE AUDIT S.A.S.` en `razonSocial`. Factura y nota de crédito transliteran nombres fiscales a un subconjunto ASCII seguro (`LEXCODE AUDIT SAS`) sin cambiar el nombre legal ni el RIDE; comprador cubierto por la misma regla. Ruff, pruebas XML y diff check verdes; promoción autorizada. Un comprobante de prueba ya `REJECTED` se archiva y sustituye por uno nuevo tras el despliegue; no se reenvía su XML firmado anterior. | 2026-08-31 |
