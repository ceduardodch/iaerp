...
| Claude (tarea programada `nomina-loop`) | ✅ COMPLETA | Servicio de periodos: borrador idempotente (borrar+reinsertar entradas, filtra empleados vigentes por fecha) y aprobación que bloquea regenerar; commit `facc794`. `release` tenía 2 commits de otra sesión (`payables`/`mcp`) no autorizados a `main` todavía, así que este commit se cherry-pickeó a `main` en vez de empujar `HEAD:main` completo — ver nota de rama abajo | 2026-08-22 |
