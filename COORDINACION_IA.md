...
| Claude (tarea programada `observabilidad-loop`) | 🟢 ACTIVA | Observabilidad y bucle de autorresolución de fallos. Cola en `docs/OBSERVABILIDAD_PENDIENTES.md`. Área: `app/api/ops.py`, `app/services/ops_failures.py`, `app/main.py` (handlers/logging), `components/action-queue/`, `main.tsx`, `ErrorBoundary.tsx`. **Se apaga sola** al cerrar la Fase 2: la Fase 3 (agente con escritura) exige autorización humana explícita | 2026-08-29 |
