...
| Claude (tarea programada `observabilidad-loop`) | 🟢 ACTIVA | Fase 1 y 2 de `docs/OBSERVABILIDAD_PENDIENTES.md` completas (pendientes 1-9), **✅ EN MAIN** (PR #62). **Fase 3 AUTORIZADA por el humano el 2026-08-31**; pendiente 10 (`ops.list_failures`, tool MCP de solo lectura sobre `ops_failures.list_failures`) retomado sin conflicto tras liberarse `app/mcp/server.py`/`tool_fingerprints.py`/`contracts/mcp-tools.yaml` (la tool `tax.process_received_reports` de la otra sesión ya estaba en `main`). Commit `2b9c269` en `release`, CI `33395454766` verde. Detalle en la Bitácora de `docs/OBSERVABILIDAD_PENDIENTES.md`. Sigue el pendiente 11 (`ops.retry_failure`) en la próxima corrida. Publica solo en `release` | 2026-08-31 |
| Codex (este hilo, reportes SRI diarios) | ✅ EN PRODUCCIÓN | Flujo local consulta ayer; REST/MCP importa de 1 a 5 TXT ya cargados y encola recuperación XML con tenant, fecha, política, idempotencia y auditoría. PR `#63`, merge `f37743b`, CI `33392739692`, Coolify, salud pública, OpenAPI y MCP protegido verdes. Falta emitir la cuenta local mínima y acordar la hora. Los cambios sin commit de `ops.list_failures` siguen intactos y fuera de esta entrega. | 2026-08-31 |
| Codex (este hilo, refresco SRI mensual) | ✅ EN MAIN Y PROBADA | Ejecuta Chrome visible por SRI en Línea, toma ambos accesos del Llavero, consulta mes de ayer con día Todos y carga los reportes a IAERP. PR `#69`, merge `af64e3d` y CI `33415359865` verdes. Dos corridas reales de agosto devolvieron el mismo conjunto de 3 reportes y 468 filas; la segunda reutilizó las claves de idempotencia por contenido, sin repetir la escritura. Tarea activa a las 08:00. | 2026-08-31 |
| Codex (este hilo, SRI multiempresa) | ✅ EN PRODUCCIÓN | `--all` procesa DATA-CLIP, BTOB SAS y LEXCODE con RUC/clave, cuenta IAERP y perfil de Chrome separados. Evidencia previa: 468 registros en DATA-CLIP y 18 en BTOB. LEXCODE quedó provisionada con cuenta `tax:write`; dos corridas reales listaron las mismas cuatro facturas de agosto y la UI conservó cuatro compras, sin duplicarlas. El preflight RUC↔tenant y la validación local de todos los receptores bloquean cuentas o sesiones cruzadas antes de guardar evidencia. Diez pruebas Node y revisión de seguridad GO. PR `#73`, merge `e194140`, CI `33427391055`, Coolify y salud pública verdes; tarea diaria activa a las 08:00. | 2026-08-31 |
| Codex (este hilo, razón social XML SRI) | ✅ EN PRODUCCIÓN | El SRI rechazó el valor `LEXCODE AUDIT S.A.S.` en `razonSocial`. Factura y nota de crédito transliteran nombres fiscales a un subconjunto ASCII seguro (`LEXCODE AUDIT SAS`) sin cambiar el nombre legal ni el RIDE; comprador cubierto por la misma regla. Revisión fiscal, CI, Coolify y salud pública verdes en PR `#73`, merge `e194140`. Un comprobante de prueba ya `REJECTED` se archiva y sustituye por uno nuevo; no se reenvía su XML firmado anterior. | 2026-08-31 |
