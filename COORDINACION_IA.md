...
| Claude (tarea programada `observabilidad-loop`) | 🟢 ACTIVA (Fase 3) | Fase 1 y 2 de `docs/OBSERVABILIDAD_PENDIENTES.md` completas (pendientes 1-9: `GET /ops/failures`, handler global de excepciones, `classify_failure()`, `POST /ops/failures/{id}/retry`, tipos y panel de Incidencias en frontend, captura de errores de frontend, E2E dedicado, Sentry/GlitchTip gateado por `IAERP_ERROR_DSN`). **✅ EN MAIN**: PR #62 (merge `db325d1`, sin squash para conservar los hashes que cita la bitácora), CI `33319491841` completo verde y despliegue Coolify 6m21s. Salud pública confirmada (`/health/startup` → `schema ok`, `environment production`) y `/api/v1/ops/failures` responde 401 en vez de 404, o sea la ruta quedó desplegada. **Fase 3 AUTORIZADA por el humano el 2026-08-31**: bucle reactivado sobre los pendientes 10-13 (tools MCP `ops.list_failures`/`ops.retry_failure`, reintento determinista en el worker y runbook de encendido). Se publica **inerte**: `_require_automation_writes` falla cerrado y `writes_enabled` sigue en `False` por tenant; el bucle tiene prohibido encenderlo en cualquier sitio. Área: `app/mcp/server.py`, `contracts/mcp-tools.yaml`, `app/workers/dispatcher.py`, `docs/10-operations.md`. Publica solo en `release` | 2026-08-31 |
