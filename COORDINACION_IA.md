...
| Claude (tarea programada `observabilidad-loop`) | ⏸️ PAUSADA (colisión) | Fase 1 y 2 de `docs/OBSERVABILIDAD_PENDIENTES.md` completas (pendientes 1-9: `GET /ops/failures`, handler global de excepciones, `classify_failure()`, `POST /ops/failures/{id}/retry`, tipos y panel de Incidencias en frontend, captura de errores de frontend, E2E dedicado, Sentry/GlitchTip gateado por `IAERP_ERROR_DSN`). **✅ EN MAIN**: PR #62 (merge `db325d1`), CI `33319491841` verde y despliegue Coolify verde. **Fase 3 AUTORIZADA por el humano el 2026-08-31**; el pendiente 10 (`ops.list_failures`) quedó implementado y verificado en local (6 pruebas nuevas, ruff/mypy limpios) pero **sin commitear**: a mitad de la corrida detecté otra sesión trabajando en vivo sobre `app/mcp/server.py`, `app/mcp/tool_fingerprints.py` y `contracts/mcp-tools.yaml` (mismos archivos que necesito) para una tool `tax.process_received_reports` no relacionada, con archivos sin commitear propios (`app/api/tax.py`, `app/services/tax/received_reports.py`, `tests/test_mcp_tax.py`, `docs/runbooks/`). Regla 1 de la sección 1 de arriba: paré sin tocar sus archivos ni forzar un commit mezclado. Detalle completo en la Bitácora de `docs/OBSERVABILIDAD_PENDIENTES.md` (2026-08-31). Si eres esa otra sesión: cuando termines y commitees tu trabajo de `tax.*`, este bucle puede retomar `ops.list_failures` sin conflicto de diseño, solo falta el commit. Publica solo en `release` | 2026-08-31 |
