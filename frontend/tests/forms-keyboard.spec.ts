...
test.describe('Form Keyboard Navigation', () => {
  ...
  test('uses space to activate buttons', async ({ page }) => {
    ...
    // prueba la activación por teclado (Space) de un botón que sí abre modal.
    const quickAdd = page.getByRole('button', { name: 'Crear lead en Nuevo' })
  ...
  test('has proper label associations', async ({ page }) => {
    ...
    for (let i = 0; i < Math.min(count, 5); i++) {
      ...
      const hasLabel = await input.evaluate(el => {
        ...
        // Label asociado por id o etiqueta envolvente (<label>texto<input></label>),
        // ambos patrones válidos. La versión anterior solo aceptaba `id`, dando
        // falsos negativos con las labels envolventes del formulario.
        const labelledById =
          el.id !== '' && document.querySelector(`label[for="${el.id}"]`) !== null
        const wrappedByLabel = el.closest('label') !== null
        return hasAriaLabel || hasAriaLabelledby || labelledById || wrappedByLabel
      })
